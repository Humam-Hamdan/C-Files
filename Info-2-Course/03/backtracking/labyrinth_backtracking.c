...
enum { N_MAX = 100,   /* maximale Labyrinth-Groesse */
       M_MAX = 100 } ;
...
void loesung( Labyrinth lab, int n, int m,
              Weg w, int laenge )
{
   int p, q ;
...
void fortsetzen( Labyrinth lab, int m, int n,
                 Feld ziel,
                 Weg w, int laenge, int k_q, int l_q )
{
   if ( ( k_q >= 0 ) && ( k_q < m ) &&
        ( l_q >= 0 ) && ( l_q < n ) ) {
