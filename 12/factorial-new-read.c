...
/* Anzahl der Dezimalstellen aus der Langzahldarstellung berechnen */
int berechneDezimalstellen(NatZahl a)
{
    if (a == NULL)
        return 0;
    return 1 + berechneDezimalstellen(a->rest);
}
      ...
      if (n % SCHRITT_N == 0)
        {
          ...
          // Anzahl der Dezimalstellen berechnen
          int dezimalstellen = berechneDezimalstellen(fak);
          printf("Dezimalstellen: %d\n", dezimalstellen);
